...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, batch_data in enumerate(
        ...
        if update_grad and (((data_iter_step + 1) // accum_iter) % n_update_per_save == 0 or (data_iter_step+1) == int(len(data_loader)*0.1) or (data_iter_step+1) == int(len(data_loader)*0.5)):
            last_save_name  = f"epoch{epoch}"
            last_iteration = data_iter_step - args.save_iteration_interval
            last_save_name += f"-iter{last_iteration}"
            last_save_dir = os.path.join(args.output_dir, last_save_name)
            cur_save_name = f"epoch{epoch}" + f"-iter{data_iter_step}"
            cur_save_dir = os.path.join(args.output_dir, cur_save_name)
            if os.path.exists(last_save_dir) and os.path.exists(cur_save_dir):
                files = os.listdir(cur_save_dir)

                # Check if any file starts with 'rank'
                if any(file.startswith('rank-specific') for file in files) and len(files)==8:
                    print("File with name starting with 'rank' exists.")
                    shutil.rmtree(last_save_dir)
                    print(f"Directory '{last_save_dir}' removed successfully.")
