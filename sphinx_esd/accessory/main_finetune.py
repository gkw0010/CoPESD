...
def auto_resume_helper(output_dir):
    ...
    checkpoints = [ckpt for ckpt in checkpoints if (ckpt.startswith('epoch'))]
    checkpoints_time = [os.path.getmtime(os.path.join(output_dir, d)) for d in checkpoints]
    print(f"All checkpoints founded in {output_dir}: {checkpoints}")
    print(f"time: {checkpoints_time}")
...
def main(args):
    ...
    if args.quant:
        ...
        promote_trainable_params_to_fp32(model)
        misc.print_param_status(model)
        if fs_init.get_data_parallel_rank() == 0 and len(args.pretrained_type)>0:
    ...
    if not args.resume:
        args.resume = auto_resume_helper(args.output_dir)
    # resume stage1
    if args.resume:
        misc.resume_stage1(args, model_without_FSDP=model)

    if 'internlm' in args.llama_type:
        TransformerBlock = type(model.llma.llm.model.layers[0])
    else:
