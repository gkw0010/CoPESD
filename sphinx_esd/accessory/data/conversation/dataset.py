...
class FinetuneDialogDataset(Dataset):
            ...
            for meta in self.config['META']:
                ...
                if "root" in meta:
                    for item in meta_l:
                        if "image" in item:
                            if 's3://' in meta['root']:
                                item['image'] = str((meta['root'])+item['image'])
                            else:   
    ...
    def get_item_func(self, index):
        ...
        for value in to_predict_values:
            ...
            # print('to_predict_values', len(tokenized_value))
        ...
        # print('max_words', max_words, padding)
        if padding > 0:
        ...
        # print('mask', torch.sum(label_mask, dim=-1))
        ...
        if image is None:
            # print('input2:', input2.shape, labels.shape, input2_mask.shape)
            # print('annotation:', data_item)
            return input2, labels, input2_mask
        else:
            # print('image and input2:', input2.shape, labels.shape, input2_mask.shape, image.shape)
