...
class DiT_Llama(nn.Module):
    ...
    def patchify_and_embed(
        ...
        if isinstance(x, torch.Tensor):
            pH = pW = self.patch_size
            B, C, H, W = x.size()
            x = x.view(B, C, H // pH, pH, W // pW, pW).permute(0, 2, 4, 1, 3, 5).flatten(3)# (B, H // pH, W // pW, C, pH, pW)->(B, H // pH, W // pW, C')
            x = self.x_embedder(x)
            x = x.flatten(1, 2) #(B,H'*W',C)

            mask = torch.ones(x.shape[0], x.shape[1], dtype=torch.int32, device=x.device)
            # leave the first line for text
            return x, mask, [(H, W)] * B, self.freqs_cis[:H//pH, :W//pW].flatten(0,1).unsqueeze(0)#(1, H'*W', dim//2)
        else:
            pH = pW = self.patch_size
            x_embed = []
            freqs_cis = []
            ...
            for img in x:
                ...
                img_size.append((H, W))
                img = img.view(C, H // pH, pH, W // pW, pW).permute(1, 3, 0, 2, 4).flatten(2)  #( H // pH, W // pW, C, pH, pW)->( H // pH, W // pW,C')
    ...
    def forward_with_cfg(self, x, t, cap_feats, cap_mask, cfg_scale, rope_scaling_factor=None, ntk_factor=None):
        # """
    ...
    def precompute_freqs_cis(
        ...
        freqs_cis_h = freqs_cis.view(end, 1, dim//4, 1).repeat(1, end, 1, 1) #(end, end, dim//4, 1)
        freqs_cis_w = freqs_cis.view(1, end, dim//4, 1).repeat(end, 1, 1, 1) #(end, end, dim//4, 1)
        freqs_cis = torch.cat([freqs_cis_h, freqs_cis_w], dim=-1).flatten(2) #(end, end, dim//4, 2)->(end, end, dim//2)
        return freqs_cis
