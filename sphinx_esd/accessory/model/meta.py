...
class MetaModel(nn.Module):
    def __init__(
        ...
        print('model args', ModelArgs)

        llama_args = {}
        if isinstance(llama_config, str):
            llama_config = [llama_config]
        for _ in llama_config:
            if len(_)>0 and os.path.exists(_):
                with open(_, "r") as f:
    ...
    def forward(self, examples, labels, images=None):
        ...
        #    c_loss = self.criterion(output.reshape(-1, vocab_size), labels.flatten())
    ...
    def generate(
        ...
        print('before max seg len:', max_seq_len)
        if images is not None:
            max_seq_len -= self.llma.image_words
        print('AFTER max seg len:', max_seq_len, self.llma.image_words, max_gen_len)
